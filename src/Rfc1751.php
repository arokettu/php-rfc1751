...
final class Rfc1751
{
    ...
    public static function encode(string $data): string
    {
        ...
        for ($i = 0; $i < \strlen($data); $i += 8) {
            $byte1 = \ord($data[$i]);
            $byte2 = \ord($data[$i + 1] ?? "\0");
            $byte3 = \ord($data[$i + 2] ?? "\0");
            $byte4 = \ord($data[$i + 3] ?? "\0");
            $byte5 = \ord($data[$i + 4] ?? "\0");
            $byte6 = \ord($data[$i + 5] ?? "\0");
            $byte7 = \ord($data[$i + 6] ?? "\0");
            $byte8 = \ord($data[$i + 7] ?? "\0");

            $parity = 0;
            for ($j = 0; $j < 4; $j++) { $parity += $byte1 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte2 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte3 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte4 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte5 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte6 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte7 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parity += $byte8 >> ($j * 2); }
            $parity &= 0b11;

            $encoded[] = self::DICT[$byte1 << 3 | $byte2 >> 5];
            $encoded[] = self::DICT[($byte2 << 6 | $byte3 >> 2) & 0x7ff];
            $encoded[] = self::DICT[($byte3 << 9 | $byte4 << 1 | $byte5 >> 7) & 0x7ff];
            $encoded[] = self::DICT[($byte5 << 4 | $byte6 >> 4) & 0x7ff];
            $encoded[] = self::DICT[($byte6 << 7 | $byte7 >> 1) & 0x7ff];
            $encoded[] = self::DICT[($byte7 << 10 | $byte8 << 2 | $parity) & 0x7ff];
        }
    ...
    public static function decode(string $encoded): string
    {
        ...
        for ($i = 0; $i < \count($words); $i += 6) {
            ...
            $byte3 = ($word2 << 2 | $word3 >> 9) & 0xff;
            $byte4 = ($word3 >> 1) & 0xff;
            $byte5 = ($word3 << 7 | $word4 >> 4) & 0xff;
            $byte6 = ($word4 << 4 | $word5 >> 7) & 0xff;
            $byte7 = ($word5 << 1 | $word6 >> 10) & 0xff;
            $byte8 = ($word6 >> 2) & 0xff;
            $parityDecoded = $word6 & 0b11;

            $parityCalc = 0;
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte1 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte2 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte3 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte4 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte5 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte6 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte7 >> ($j * 2); }
            for ($j = 0; $j < 4; $j++) { $parityCalc += $byte8 >> ($j * 2); }
            $parityCalc &= 0b11;
            ...
            $decoded .=
                ...
                \chr($byte5) . \chr($byte6) . \chr($byte7) . \chr($byte8);
        }
